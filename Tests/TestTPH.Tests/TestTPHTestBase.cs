...
namespace TestTPH.Tests
{
    public abstract class TestTPHTestBase : AbpIntegratedTestBase<TestTPHTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected TestTPHTestBase()
        {
        ...
        protected void UsingDbContext(Action<TestTPHDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TestTPHDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TestTPHDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TestTPHDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TestTPHDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestTPHDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TestTPHDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestTPHDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TestTPHDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TestTPHDbContext, Task<T>> func)
        {
            T result;
